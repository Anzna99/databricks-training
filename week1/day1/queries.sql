...
19. SELECT department_id, COUNT(*) 
FROM Employee 
GROUP BY department_id;
20. SELECT department_id, AVG(salary) 
FROM Employee 
GROUP BY department_id;
## groupBy Queries
21. SELECT department_id, SUM(salary) 
FROM Employee 
GROUP BY department_id;
22. SELECT department_id, AVG(age) 
FROM Employee 
GROUP BY department_id;
23. SELECT YEAR(hire_date), COUNT(*) 
FROM Employee 
GROUP BY YEAR(hire_date);
24. SELECT department_id, MAX(salary) 
FROM Employee 
GROUP BY department_id;
25. SELECT department_id, AVG(salary) AS avg_sal
FROM Employee
GROUP BY department_id
ORDER BY avg_sal DESC
LIMIT 1;
## havingqueries
26. SELECT department_id, COUNT(*) 
FROM Employee 
GROUP BY department_id
HAVING COUNT(*) > 2;
...
28. SELECT YEAR(hire_date), COUNT(*) 
FROM Employee 
GROUP BY YEAR(hire_date)
HAVING COUNT(*) > 1;
29. SELECT department_id, SUM(salary) 
FROM Employee 
GROUP BY department_id
HAVING SUM(salary) < 100000;

30. SELECT department_id, MAX(salary) 
FROM Employee 
GROUP BY department_id
...
##JoinQueries
36. SELECT e.name, d.name AS department_name
FROM Employee e
JOIN Department d ON e.department_id = d.department_id;
37. SELECT p.name AS project_name, d.name AS department_name
FROM Project p
JOIN Department d ON p.department_id = d.department_id;
38. SELECT e.name AS employee, p.name AS project
FROM Employee e
JOIN Project p ON e.department_id = p.department_id;
39. SELECT e.name, d.name AS department
FROM Employee e
LEFT JOIN Department d ON e.department_id = d.department_id;
40. SELECT d.name, e.name
FROM Department d
...
42. SELECT e.name, COUNT(p.project_id) AS project_count
FROM Employee e
JOIN Project p ON e.department_id = p.department_id
GROUP BY e.emp_id;
43. SELECT d.name
...
WHERE hire_date >= DATE_SUB(CURDATE(), INTERVAL 2 YEAR)
ORDER BY hire_date;
60. SELECT department_id, COUNT(*) AS total_emp, AVG(salary)
FROM Employee
GROUP BY department_id
HAVING COUNT(*) > 2;
61. SELECT e.name, e.salary
FROM Employee e
WHERE salary > (
    SELECT AVG(salary)
    FROM Employee
...
64. SELECT e.name, e.salary, e.department_id
FROM Employee e
WHERE salary = (
    SELECT MAX(salary)
    FROM Employee
